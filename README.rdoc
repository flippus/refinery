...
I advise to change the +show+ method to the following, so you have a redirecting to the index path of the event, if the id does not exist.
...
But don't forget, there's a +find_object+ method in every controller, which you maybe have to override.
